...
def has_pending_commit_resolution(group):
    ...
    if ReleaseCommit.objects.filter(commit__id=recent_group_link.linked_id).exists():
        ...
        if PullRequestCommit.objects.filter(commit__id=recent_group_link.linked_id).exists():
            # check if any commits in PR have been released
            all_commits = PullRequestCommit.objects.get_queryset()
            for commit in all_commits:
                if ReleaseCommit.objects.filter(commit__id=commit.id).exists():
                    return False
        return True
