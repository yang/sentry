...
class OrganizationRole(Role):
    ...
    def can_manage(self, other: str) -> bool:
        other_role = self.parent.get(other)
...
class RoleManager:
    def __init__(
        ...
        for idx, role_cfg in enumerate(org_config):
            role = OrganizationRole(self, idx, **role_cfg)
        ...
        for idx, role_cfg in enumerate(team_config):
            role = TeamRole(self, idx, **role_cfg)
    ...
    def can_manage(self, role: str, other: str) -> bool:
        return self.get(role).can_manage(other)
